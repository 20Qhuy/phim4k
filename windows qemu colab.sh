...
qemu-system-x86_64 -M q35 -usb -device qemu-xhci -device usb-tablet -device usb-kbd -cpu host,+pae -smp sockets=1,cores=8,threads=1 -m 32G -drive file=win.qcow2,aio=threads,cache=writeback,if=none,id=hda -vga std -device ich9-intel-hda -device hda-duplex -device e1000e,netdev=n0 -netdev user,id=n0 -accel tcg -device virtio-serial-pci -device intel-iommu -vnc :0
